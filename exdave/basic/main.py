...
class ExampleBenchmark(Benchmark):
    ...
    def setup(self, env: Environment):
        ...
        containers.put(
            ImageProperties(
                "resnet50-inference-gpu", parse_size_string("56M"), arch="arm32"
            )
        )
        containers.put(
            ImageProperties(
                "resnet50-inference-gpu", parse_size_string("56M"), arch="x86"
            )
        )
        containers.put(
            ImageProperties(
                "resnet50-inference-gpu", parse_size_string("56M"), arch="aarch64"
            )
        )
    ...
    def run(self, env: Environment):
        ...
        for i in range(10):
            ps.append(
                env.process(env.faas.invoke(FunctionRequest("python-pi", env.now)))
            )
        ...
        for i in range(10):
            ps.append(
                env.process(
                    env.faas.invoke(FunctionRequest("resnet50-inference", env.now))
                )
            )
    ...
    def prepare_resnet_inference_deployment(self):
        ...
        resnet_fn = Function(
            resnet_inference, fn_images=[resnet_inference_gpu, resnet_inference_cpu]
        )
        ...
        resnet_cpu_container = FunctionContainer(
            resnet_inference_cpu, SimResourceConfiguration()
        )
        ...
        resnet_gpu_container = FunctionContainer(
            resnet_inference_gpu, resource_config=request
        )
