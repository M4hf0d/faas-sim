...
def main():
    ...
    sim = Simulation(
        basic.example_topology(),
        TrainInferenceBenchmark(inference_factory, train_factory),
    )
    ...
    dfs = {
        ...
        "function_utilization_df": sim.env.metrics.extract_dataframe(
...
class TrainInferenceBenchmark(Benchmark):
    ...
    def run(self, env: Environment):
        ...
        ia_generator = expovariate_arrival_profile(
            constant_rps_profile(rps=max_rps_inference), max_ia=1
        )
        ...
        # generate profile
        ia_generator = expovariate_arrival_profile(
            constant_rps_profile(rps=max_rps_training), max_ia=1
        )
    ...
    def prepare_resnet_training_deployment(self):
        ...
        resnet_cpu_container = FunctionContainer(
            resnet_training_cpu, SimResourceConfiguration()
        )
