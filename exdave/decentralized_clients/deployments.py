...
def prepare_client_deployment(
    ...
    client_container = ClientFunctionContainer(
        fn_container,
        ia_generator,
        request_factory,
        deployment,
...
def prepare_resnet_training_deployment():
    ...
    resnet_fn = Function(
        resnet_training,
        fn_images=[resnet_training_cpu],
        labels={function_label: resnet_training},
    )
    ...
    resnet_cpu_container = FunctionContainer(
        ...
        SimResourceConfiguration(),
        labels={
            function_label: resnet_training,
            worker_role_label: "true",
            pod_type_label: function_type_label,
        },
    )
...
def prepare_client_deployments(
    ...
    for idx, client in enumerate(client_names):
        client_inference_fd = prepare_client_deployment(
            str(idx), client, ia_generator, max_requests, request_factory, deployment
        )
...
def prepare_inference_clients(
    clients: List[str], request_factory, deployment: SimFunctionDeployment
