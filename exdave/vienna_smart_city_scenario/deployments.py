...
from faas.util.constant import (
...
def prepare_resnet_inference_deployment(min_scale: int):
    ...
    resnet_fn = Function(
        resnet_inference,
        fn_images=[resnet_inference_cpu],
        labels={function_label: resnet_inference},
    )
    ...
    resnet_cpu_container = FunctionContainer(
        ...
        {
            worker_role_label: "true",
            function_label: resnet_inference,
    ...
    resnet_fd = SimFunctionDeployment(
        ...
        SimScalingConfiguration(
            ScalingConfiguration(scale_max=1000, scale_min=min_scale)
        ),
...
def create_load_balancer_deployment(lb_id: str, type: str, host: str, cluster: str):
    ...
    lb_fd = SimFunctionDeployment(
        ...
        [lb_container],
        SimScalingConfiguration(
            ScalingConfiguration(scale_min=1, scale_max=1, scale_zero=False)
        ),
...
def get_resnet50_inference_cpu_image_properties():
    return [
        ...
        ImageProperties(
            "faas-workloads/resnet-inference-cpu", parse_size_string("56M"), arch="x86"
        ),
        ImageProperties(
            "faas-workloads/resnet-inference-cpu",
            parse_size_string("56M"),
...
def prepare_load_balancer_deployments(
    type: str, hosts: List[Node]
    ...
    for idx, host in enumerate(hosts):
        lb_id = create_id(idx)
        lbs.append(
            create_load_balancer_deployment(
                lb_id, type, host.name, host.labels[zone_label]
            )
        )
...
def prepare_client_deployments_for_experiment(
    ...
    max_requests,
    clients,
    lb_deployments: List[SimFunctionDeployment],
...
def prepare_client_deployment(
    ...
    lb_deployment: SimFunctionDeployment,
):
    # Design time
    ...
    fn_container = FunctionContainer(
        client_image,
        SimResourceConfiguration(),
        {client_role_label: "true", hostname_label: host},
    )
...
def prepare_client_deployments(
    ia_generator,
...
def prepare_inference_clients(
    ...
    # generate profile
    inference_ia_generator = expovariate_arrival_profile(
        constant_rps_profile(rps=max_rps), max_ia=1
    )
    logger.info(
    ...
    inference_client_fds = prepare_client_deployments(
        inference_ia_generator, clients, deployment, lb_deployment, max_requests, size
    )
    return inference_client_fds


def prepare_function_deployments(
    resnet_inference_scale_min: int,
) -> List[SimFunctionDeployment]:
    resnet_inference_fd = prepare_resnet_inference_deployment(
        resnet_inference_scale_min
    )
