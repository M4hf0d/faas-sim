...
from exdave.vienna_smart_city_scenario.deployments import (
...
from exdave.vienna_smart_city_scenario.predicates import (
...
class DecentralizedAIFunctionSimulatorFactory(SimulatorFactory):
    ...
    def __init__(
        self,
        load_balancer_factory: Callable[[Environment, FunctionContainer], Any],
...
class ViennaSmartCityBenchmark(Benchmark):
    def __init__(
        ...
        self.metadata = {
            ...
            "benchmark": "ViennaSmartCityBenchmark",
            "resnet_inference_scale_min": resnet_inference_scale_min,
            "districts": districts,
        }
    ...
    def run(self, env: Environment):
        ...
        function_deployments = prepare_function_deployments(
            self.resnet_inference_scale_min
        )
...
def execute_benchmark(
    max_rps: int, max_requests: int, districts: int, resnet_inference_scale_min: int
):
    ...
    benchmark = ViennaSmartCityBenchmark(
        max_rps, max_requests, districts, resnet_inference_scale_min, clients, lbs
    )
    ...
    sim.env.simulator_factory = DecentralizedAIFunctionSimulatorFactory(
        create_load_balancer, fet_oracle, resource_oracle
    )
...
def main():
    ...
    duration, sim = execute_benchmark(
        max_rps, max_requests, districts, resnet_inference_scale_min
    )
    env = sim.env
    dfs = extract_dfs(sim)
