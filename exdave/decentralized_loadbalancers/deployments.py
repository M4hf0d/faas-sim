...
from exdave.decentralized_clients.deployments import (
    prepare_resnet_inference_deployment,
...
def prepare_client_deployments_for_experiment(
    ...
    fds.extend(
        prepare_inference_clients(
            inference_clients, inference_size, deployments[0], lb_deployments[0]
        )
    )
    fds.extend(
        prepare_training_clients(
            training_clients, training_size, deployments[1], lb_deployments[2]
        )
    )
...
def prepare_inference_clients(
    ...
    inference_ia_generator = expovariate_arrival_profile(
        constant_rps_profile(rps=inference_max_rps), max_ia=1
    )
    logger.info(
...
def prepare_training_clients(
    ...
    # generate profile
    training_ia_generator = expovariate_arrival_profile(
        constant_rps_profile(rps=training_max_rps), max_ia=1
    )
