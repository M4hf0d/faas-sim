...
def main():
    ...
    for k, df in dfs.items():
        df.to_csv(f"data/output/{nodename}/resnet-inference-cpu/{k}.csv", index=False)
...
class PowerPredictionBenchmark(Benchmark):
    ...
    def setup(self, env: Environment):
        ...
        containers.put(
            ImageProperties(
                "resi5/resnet-inference-cpu", parse_size_string("2710M"), arch="x86"
            )
        )
        containers.put(
            ImageProperties(
                "resi5/resnet-inference-cpu", parse_size_string("1030M"), arch="aarch64"
            )
        )
    ...
    def prepare_deployments(self, env: Environment) -> List[FunctionDeployment]:
        ...
        for i in range(n):
            function_deployment = self.prepare_deployment(
                env, f"resnet-inference-{i}", "resi5/resnet-inference-cpu", i
            )
