...
def main():
    ...
    for k, df in dfs.items():
        df.to_csv(f"data/output/{nodename}/resnet-inference-gpu/{k}.csv", index=False)
...
class PowerPredictionBenchmark(Benchmark):
    ...
    def setup(self, env: Environment):
        ...
        containers.put(
            ImageProperties(
                "resi5/resnet-inference-gpu", parse_size_string("2710M"), arch="x86"
            )
        )
        containers.put(
            ImageProperties(
                "resi5/resnet-inference-gpu", parse_size_string("1030M"), arch="aarch64"
            )
        )
