...
from ext.tmueller23.functionsim import (
    PowerPredictionSimulatorFactory,
    client_node_label,
    size_label,
)
...
def main():
    ...
    dfs = {
        ...
        "simulation_duration_df": sim.env.metrics.extract_dataframe(
            "simulation_duration"
        ),
        "prediction_time_df": sim.env.metrics.extract_dataframe("prediction_time"),
    }

    for k, df in dfs.items():
        df.to_csv(f"data/output/{k}.csv", index=False)  # TODO: change output folder
...
class PowerPredictionBenchmark(Benchmark):
    ...
    def setup(self, env: Environment):
        ...
        containers.put(
            ImageProperties(
                "resi5/resnet-inference-cpu", parse_size_string("2.7G"), arch="x86"
            )
        )
        containers.put(
            ImageProperties(
                "resi5/resnet-inference-cpu", parse_size_string("1G"), arch="aarch64"
            )
        )

        containers.put(
            ImageProperties(
                "resi5/resnet-inference-gpu", parse_size_string("2.7G"), arch="x86"
            )
        )
        containers.put(
            ImageProperties(
                "resi5/resnet-inference-gpu", parse_size_string("1G"), arch="aarch64"
            )
        )

        containers.put(
            ImageProperties(
                "resi5/efficientnet-inference-cpu",
                parse_size_string("2.6G"),
                arch="x86",
            )
        )
        containers.put(
        ...
        containers.put(
            ImageProperties(
                "resi5/efficientnet-inference-gpu",
                parse_size_string("2.6G"),
                arch="x86",
            )
        )
        ...
        containers.put(
            ImageProperties(
                "edgerun/objectdetection-cpu", parse_size_string("970M"), arch="x86"
            )
        )
        containers.put(
            ImageProperties(
                "edgerun/objectdetection-cpu", parse_size_string("828M"), arch="aarch64"
            )
        )
    ...
    def run(self, env: Environment):
        ...
        for deployment in deployments:
            yield env.process(env.faas.poll_available_replica(deployment.fn.name))
    ...
    def prepare_deployments(self, env: Environment) -> List[FunctionDeployment]:
        # resnet_inference_fd = self.prepare_resnet_inference_cpu_deployment(env)

        deployments = []
        n = int(
            (len(env.topology.get_nodes()) - 1) / 2
        )  # -1 because of registry node, /2 because of node/nuc tuples

        logger.debug(f"DEBUG nodes: {env.topology.get_nodes()}")
        logger.debug(f"DEBUG number of node tuples: {n}")

        for i in range(n):
            function_deployment = self.prepare_deployment(
                env, f"resnet-inference-{i}", "resi5/resnet-inference-gpu", i
            )
            deployments.append(function_deployment)
        # function_deployment = self.prepare_deployment(env, 'resnet-inference', 'resi5/resnet-inference-gpu')
        # function_deployment = self.prepare_deployment(env, 'efficientnet-inference', 'resi5/efficientnet-inference-cpu')
        # function_deployment = self.prepare_deployment(env, 'efficientnet-inference', 'resi5/efficientnet-inference-gpu')
        # function_deployment = self.prepare_deployment(env, 'objectdetection', 'edgerun/objectdetection-cpu')

        return deployments

    def prepare_resnet_inference_cpu_deployment(self, env: Environment):
        # Design Time
        resnet_inference = "resnet-inference"
        resnet_inference_cpu = FunctionImage(image="resi5/resnet-inference-cpu")
        resnet_inference_fn = Function(
            resnet_inference, fn_images=[resnet_inference_cpu]
        )
        ...
        size = parse_size_string("1M")
        resnet_inference_fn_container = FunctionContainer(
        ...
        resnet_inference_fd = FunctionDeployment(
            resnet_inference_fn, [resnet_inference_fn_container], ScalingConfiguration()
        )

        return resnet_inference_fd

    def prepare_deployment(
        self, env: Environment, function_name: str, function_image: str, i
    ):
        # Design Time
        ...
        function_deployment = FunctionDeployment(
            function, [function_container], ScalingConfiguration()
        )
...
def get_tmueller23_function_characterizations(
    resource_oracle: ResourceOracle, fet_oracle: FetOracle
